...
def ingest(state: TriageState) -> TriageState:
    ...
    state.messages.append(f"Ticket received: {state.ticket_text}")
    return state


def extract_order_id(state: TriageState) -> TriageState:
    """Assistant extracts order ID from ticket text"""
...
def classify_issue(state: TriageState) -> TriageState:
    """Assistant classifies the issue and suggests a recommendation"""
...
def fetch_order(state: TriageState) -> TriageState:
    """Assistant fetches order details from mock data"""
    if state.order_id:
...
def draft_reply(state: TriageState) -> TriageState:
    """Assistant drafts a response for the customer"""
    template = next(
    ...
    if template:
        ...
        state.messages.append("No template found; drafted generic reply.")
...
def admin_review(state: TriageState) -> TriageState:
    """Admin approves or flags the assistant's suggested action"""
    if state.recommendation == "review":
        state.messages.append("Admin: flagged for review (manual approval needed).")
    else:
        state.messages.append("Admin: approved the suggested action.")
    return state
